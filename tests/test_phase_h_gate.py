...
def test_phase_h_gate_passes_current_thresholds(tmp_path: Path) -> None:
    result = evaluate_phase_h_gate(tmp_path / "phase_h_gate.sqlite3")
...
def test_phase_h_gate_report_writes_json(tmp_path: Path) -> None:
    result = evaluate_phase_h_gate(tmp_path / "phase_h_report.sqlite3")

    output_path = write_phase_h_gate_report_json(tmp_path / "phase_h_report.json", result)
    payload = json.loads(output_path.read_text(encoding="utf-8"))

    assert payload["schema_version"] == "phase_h_gate_report_v1"
    assert payload["phase_h_pass"] is True
