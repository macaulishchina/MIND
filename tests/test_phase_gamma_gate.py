"""Phase γ gate tests: new object types, graph retrieval, model routing, artifact memory, auto-archive.
...
class TestGammaGraphRetrieval:
    ...
    def test_scheduler_schedule_discover_links(self) -> None:
        ...
        jobs: list = []
        class FakeStore:
            def enqueue_offline_job(self, job): jobs.append(job)
...
class TestGammaModelRouting:
    ...
    def test_backward_compat_without_routing(self) -> None:
        ...
        assert service.invoke(req).summary_text
...
class TestGammaArtifactMemory:
    ...
    def test_artifact_index_schema(self) -> None:
        from mind.kernel.schema import CORE_OBJECT_TYPES, REQUIRED_METADATA_FIELDS, validate_object
        assert "ArtifactIndex" in CORE_OBJECT_TYPES
...
class TestGammaAutoArchive:
    ...
    def _job(self, payload):
        from mind.offline_jobs import new_offline_job, OfflineJobKind
    ...
    def test_archive_report_gate_pass(self) -> None:
        ...
        report = ArchiveReport.compute(
            archived_count=20, unarchived_count=1, total_objects=200
        )
    ...
    def test_archive_report_gate_fail(self) -> None:
        from mind.eval.growth_metrics import ArchiveReport
        report = ArchiveReport.compute(
            archived_count=10, unarchived_count=3, total_objects=100
        )
...
class TestGammaNoRegression:
    ...
    def test_beta_embedding_provider(self) -> None:
        from mind.kernel.embedding import EmbeddingProvider, LocalHashEmbedding
    ...
    def test_beta_conflict_detection(self) -> None:
        from mind.primitives.conflict import ConflictRelation, detect_conflicts
        assert ConflictRelation.DUPLICATE == "duplicate"

    def test_beta_slot_allocation_policy(self) -> None:
        from mind.workspace.policy import FLASH_POLICY, SlotAllocationPolicy
    ...
    def test_alpha_growth_phase_alpha_report(self) -> None:
        from mind.eval.growth_metrics import GrowthPhaseAlphaReport, ArchiveReport
