...
def test_phase_g_cost_report_round_trips(tmp_path: Path) -> None:
    ...
    output_path = write_phase_g_cost_report_json(tmp_path / "phase_g_cost_report.json", report)
    reloaded = read_phase_g_cost_report_json(output_path)
...
def test_phase_g_cost_report_freezes_fixed_rule_budget_profile() -> None:
    ...
    assert report.schema_version == "phase_g_cost_report_v1"
    assert report.strategy_id == "fixed_rule_v1"
    assert report.budget_profile.profile_id == "phase_g_fixed_rule_budget_v1"
