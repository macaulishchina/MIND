...
时点说明：这份文档记录的是 Phase J 通过后，MIND 进入 `Phase K / Persona / Projection` 前的启动约束、任务拆分和范围控制。正式通过口径以后续 Phase K 验收报告为准；这里先冻结启动顺序，避免把人格层工程化过早推成一个脱离记忆证据的黑箱模块。

## 目标

Phase K 做 persona projection，不造黑箱人格对象。
...
1. `K1`：冻结 `PersonaProjectionBench v1` 与 identity bundle 结构
2. `K2`：实现 autobiographical grouping 与 preference / value schema
3. `K3`：实现 runtime persona projection 与 evidence trace
4. `K4`：实现治理后的 persona 更新与泄露回归
5. `K5`：补 Phase K gate、验收报告和 neutral-task A/B 回归

## 推荐推进顺序

### `K1` 基准与 bundle 冻结

- 把 [../foundation/phase_gates.md](../foundation/phase_gates.md) 中的 `K-1 ~ K-6` 作为唯一 formal gate
...
- 依赖 Phase J 已经稳定的 governance / reshape 执行能力
